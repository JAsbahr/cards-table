...
export default class CardFormFromPuca extends React.Component {
    constructor(props) {
        super(props)
    ...
    onSubmit = (e) => {
        ...
        if (!this.state.description || !this.state.amount || !this.state.amount) {
            this.setState(() => ({
                error: "Please provide card name, amount and puca value"
            }))
        } else {
            this.setState(() => ({ error: "" }))
            this.props.onSubmit({
                description: this.state.description,
                euro: this.state.euro === "" ? 0 : parseFloat(this.state.euro) * 100,
                amount: parseFloat(this.state.amount),
                sent: this.state.sent === "" ? 0 : parseFloat(this.state.sent),
                puca: parseFloat(this.state.puca)
            })
        }
    }
